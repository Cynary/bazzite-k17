...
install -m0644 /usr/share/k17-frl/build/{xe,drm_display_helper,xone_dongle}.ko "/usr/lib/modules/$kver/updates/k17/"
...
depmod -a "$kver"
for mod in xe drm_display_helper xone_dongle; do
...
dracut --no-hostonly --kver "$kver" --reproducible --zstd --add ostree --add fido2 \
 --add-drivers 'xe drm_display_helper xone_dongle mei_me mei_gsc_proxy' -f "/usr/lib/modules/$kver/initramfs.img"
...
rm /usr/share/k17-frl/build/{xe,drm_display_helper,xone_dongle}.ko
